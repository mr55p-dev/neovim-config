...
function M.fugitive.setup()
	vim.keymap.set("n", "<Leader>gg", ":Git<CR>", { silent = true, desc = "Open Git" })
...
function M.gh()
	...
	require("litee.gh").setup({
		keymaps = {
			...
			submit_comment = "<C-s>",
			-- inside a convo buffer, when your cursor is on top of a comment, open
			-- up a set of actions that can be performed.
			actions = "<C-a>",
			-- inside a thread convo buffer, resolve the thread.
			resolve_thread = "<C-r>",
			-- inside a gh.nvim panel, if possible, open the node's web URL in your
			-- browser. useful particularly for digging into external failed CI
			-- checks.
			goto_web = "gx",
		},
	})
